

export const DummyProducts = [
    {
        id: 1,
        title: 'Modern Mango Wood Teak Finish Wooden Bed',
        image: 'https://homesofrajasthan.com/wp-content/uploads/2023/05/BD67P-700x700.jpeg',
        type: 'bed',
        price:  36000,
        crossprice: '',
        description: `100% Handmade and Stylish Bed.
        Made with premium quality Mango Wood.
        Unique carving process.
        Contemporary yet a classic design.`,
        qty: 1
    },
    {
        id: 2,
        title: 'Relexo Luxury Upholstered Bed in Leatherette',
        image: 'https://encrypted-tbn1.gstatic.com/shopping?q=tbn:ANd9GcRQTjZt_2icSQDyBV3OH_HUPGXVAFRKYLZstSGA93zJ32XICXM9ts3hjSwM7WRcygfipNP9gK2gY5hX0CqUwhklMSXLbd5IB9_20BPkQEg&usqp=CAE',
        type: 'bed',
        price: 52999,
        crossprice: '',
        description: `Nice Maple offers the best in comfort with elan. The collections are a series of modern trendy designs, simple yet striking and represent the ideals of minimalism.`,
        qty: 1
    },
    {
        id: 3,
        title: 'Jacklin Fabric King Size Bed',
        image: 'https://ii1.pepperfry.com/media/catalog/product/j/a/1100x1210/jacklin-king-size-bed-in-cream-colour-by-durian-jacklin-king-size-bed-in-cream-colour-by-durian-dalq6a.jpg',
        type: 'bed',
        price: 39780,
        crossprice: '',
        description: `Brand: Durian\nPrimary Material: Fabric\nProduct Rating: 4.5\nRoom Type: Bedroom\nStorage: No Storage\nWarranty: 5 Years' Warranty`,
        qty: 1
    },
    {
        id: 4,
        title: 'Desket Lagos Upholstered Bed',
        image: 'https://desket.in/cdn/shop/products/Screenshot2022-02-14101501_720x.jpg?v=1644817022',
        type: 'bed',
        price: 27930,
        crossprice: '',
        description: `This Desket Lagos Upholstered Bed -- which is healthy and environmentally friendly, without odor, and enjoys a quality of life. The widened, thickened and encrypted row frame is strong and stable.`,
        qty: 1
    },
    {
        id: 5,
        title: 'Golden Hexagon Metal Wall Clock',
        image: 'https://dekorspace.com/cdn/shop/files/dk-539-1.jpg?v=1689316294',
        type: 'decor',
        price: 27930,
        crossprice: '',
        description: `A wall clock which makes any room stand out. The black & golden touch
        makes it one of a kind wall clock, the elegance of which is class apart.
        It is more of a cherished piece of decor which acts as a statement piece
        at your place. A cleanly painted wall needs nothing more than a striking wall
        clock which sets aside all other decor forms. An ideal piece for home,
        offices and cafes.`,
        qty: 1
    },
    {
        id: 6,
        title: 'Rewrite The Night Grand ',
        image: 'https://www.whiteteak.com/media/catalog/product/h/l/hl64-10002_14__1.jpg?width=562&height=560&canvas=562,560&optimize=high&fit=bounds',
        type: 'lighting',
        price: 69200,
        crossprice: '',
        description: `Intricately crafted shapes suspended through ultra-thin aircraft cables and
        inbuilt LED’s, she’s the stunner that’s set to dazzle you like the stars.
        The 3 built-in LED lights can be brightened or dimmed with the Remote
        Control unit which comes along with this chandelier.`,
        qty: 1
    },
    {
        id: 7,
        title: 'L shaped Sofa',
        image: 'https://i0.wp.com/shopps.in/wp-content/uploads/2023/07/H148c1d4c98ec48fd8abc4ee54c9d95e50.jpg?resize=504%2C504&ssl=1',
        type: 'sofa',
        price: 190000,
        crossprice: '',
        description: `An L shape sofa, as the name suggests, is designed in the
        shape of the letter “L.” It provides ample seating space for family and
        guests while optimizing the available area in your living room. The
        extended chaise lounge offers an added layer of comfort, perfect for
        relaxation after a long day.`,
        qty: 1
    },
    {
        id: 8,
        title: 'Tanjavore Dining table',
        image: 'https://i0.wp.com/shopps.in/wp-content/uploads/2023/01/1b2cfdea3bc542aaa420d0731112401a.webp?resize=504%2C504&ssl=1',
        type: 'table',
        price: 12500,
        crossprice: '',
        description: `This table is made of the highest quality stainless steel and finished with
        a beautiful coating. It is rust proof, scratch resistant, and easy to clean.
        Plus, it comes in a variety of colours to match any decor.`,
        qty: 1
    },
    {
        id: 9,
        title: 'Astor Modern 4 Door Wardrobe',
        image: 'https://ii1.pepperfry.com/media/catalog/product/m/o/1100x1210/modern-living-astor-4-door-wardrobe-in-grey-colour-by-spacewood-modern-living-astor-4-door-wardrobe--yws65k.jpg',
        type: 'cupboard',
        price: 68183,
        crossprice: '',
        description: `Brand :Spacewood
        Assembly :Carpenter Assembly
        Dimensions (In Centimeters) :H 211 x W 180 x D 56
        Dimensions (in Inches) :H 83 x W 71 x D 22
        Primary Material :Engineered Wood
        Room Type :Bedroom
        Warranty :36 Months' Warranty`,
        qty: 1
    },
    {
        id: 10,
        title: 'KALSIGIFTHOUSE Coir Mattress',
        image: 'https://rukminim2.flixcart.com/image/416/416/xif0q/mattress/y/e/6/72x72x47-inches-coir-mattress-m108-double-bed-matters-original-imagmhakbudq5uhe.jpeg?q=70&crop=false',
        type: 'matresses',
        price: 900,
        crossprice: 1000,
        description: `Brand : KALSIGIFTHOUSE
        Color :White
        Type : Double Bed Matters`,
        qty: 1
    },
    {
        id: 11,
        title: ' Automatic Coffee Machine ',
        image: 'https://encrypted-tbn0.gstatic.com/shopping?q=tbn:ANd9GcSveDxyC1_XnthrAlfkwuKVKDkYTEoaX-cqeu_JbxKckZgbWx-WX6LLb3ejRZWyP_LU4ko3ciwRFTjeO6wGoR8Xt_3dePVrF6XTs5ZjNdM&usqp=CAE',
        type: 'appliances',
        price: 89999,
        crossprice: '1,25,000',
        description: `One-Touch Coffee Experience: The Wonderchef Regalia Fully-Automatic Coffee Machine allows you to enjoy your favorite coffee with a simple touch of a button. Create the perfect espresso shot from fresh coffee beans and add frothed milk to your cup effortlessly.`,
        qty: 1
    },
    {
        id: 12,
        title: 'KGM 90- Cooking Range with Electric Oven, Auto',
        image: 'https://encrypted-tbn3.gstatic.com/shopping?q=tbn:ANd9GcSJDp721pe_4JQlY_rendIwnkhnp8FkmMDYCCt6NuBmmCMPo8qdSp9--xcC8a1fo22IhHNaG8YdH7xHI01l2EkwBybKL1hoNNXTN8KgoYta3g0WIjsSYiH6hg&usqp=CAE',
        type: 'appliances',
        price: 112990,
        description: `• 90cm Cooking Range with Electric Oven • Stainless Steel Finish
        • 5 Gas with 1 Big Triple Ring Sabaf Burners • Auto Electric Ignition
        • Heavy Duty Cast Iron Pan Support
        • Double Glass Oven Door with Mirror Finish
        • With Mechanical Timer`,
        qty: 1
    },
    {
        id: 13,
        title: 'Newport 2-Piece Chaise Sectional',
        image: 'https://cdn.pixelspray.io/v2/black-bread-289bfa/TIw66q/wrkr/t.resize(h:780,w:780)/data/Westelm/23052023img/8622958_1.jpg',
        type: 'sofa',
        price: 375000,
        crossprice: '₹ 4,00,000',
        description: `Solid pine and engineered hardwood frame with reinforced joinery.
        Ash wood legs. All wood is kiln-dried to prevent warping.
        Back & seat cushions are 70% poly fiber, 30% duck feather in down proof ticking.
        Seat firmness: Soft. On a scale from 1 to 5 (5 being firmest), it's a 2.`,
        qty: 1
    },
    {
        id: 14,
        title: 'Full sofa set',
        image: 'https://i0.wp.com/shopps.in/wp-content/uploads/2022/12/1_1581689237_7f-1000x928-1.jpg?resize=504%2C468&ssl=1',
        type: 'sofa',
        price: 210000,
        crossprice: '₹530,000',
        description: `set of 5 pieces
        Dispatch time 2 weeks
        delivery all India
        customisation available`,
        qty: 1
    },
    {
        id: 15,
        title: 'shimla center table',
        image: 'https://i0.wp.com/shopps.in/wp-content/uploads/2023/11/353402739_739552658180343_7288948818171543494_n.jpg?resize=504%2C504&ssl=1',
        type: 'table',
        price: 12500,
        crossprice: '₹13,000',
        description: `The Shimla Center Table, a premium SS frame marble top center table set, does exactly that. Crafted with precision in India by Blue Ribbon and available exclusively on shopps.in, this table is a symbol of luxury and sophistication.
        colours available: gold, rose gold, chrome, black
        all India home delivery 2 weeks dispatch`,
        qty: 1
    },
    {
        id: 16,
        title: 'Modern Rectangle White Marble Dining Table',
        image: 'https://encrypted-tbn2.gstatic.com/shopping?q=tbn:ANd9GcQZ800xDxhrpFJCHLxN2XDydmQnPuraXeQ4zCdpIfrJH5V59SOCCjj5d2hhQrH4rpCmMcoI6E8NSXgoGZNLajn62NFmXH5mhBRww_AAbFP4&usqp=CAE',
        type: 'table',
        price: 58999,
        crossprice: '₹60,000',
        description: `Featuring the modern style, this dining table is the perfect addition to improve your lifestyle. Rectangle-shaped tabletop is designed to showcase the beauty of faux marble, which will bring you an earthy yet graceful touch. While the trestle base reinforces its aesthetics with clean lines and gleaming gold hue. Durably constructed of high-quality materials.`,
        qty: 1
    },
    {
        id: 17,
        title: '4 Door Wardrobes',
        image: 'https://encrypted-tbn3.gstatic.com/shopping?q=tbn:ANd9GcSfqcaJ3KDgV2U6bNYisFBnO5uxzTkFSpicTeRTNGR97bnjmu2KdUiLN3hXyNIq_4o_IV9rsPjn4T0mcG69vHgktj_qVcV6esv_frrOli0&usqp=CAE',
        type: 'cupboard',
        price: 89999,
        crossprice: '₹ 166,999',
        description: `This 4 doors wardrobe Includes; 11 fixed shelves, 1 aluminum hanging rod and 2 drawers. The wardrobe is made of high-quality melamine faced chipboard finished in black marble-look and walnut colour. Assembly elements and guide are included in the box. It has an uncomplicated assembly structure for your convenience.`,
        qty: 1
    },
    {
        id: 18,
        title: 'Nordic Wardrobe Modern Minimalist – Small Wide Wardrobe',
        image: 'https://encrypted-tbn2.gstatic.com/shopping?q=tbn:ANd9GcS6vtx9bpMO6zhOM82bsqoYIg18jbJCGsKPrydD7RZNd4Hzf-pOVX0SR3m01yQMdratDBKy77IOUhReyqxo8JUZkUTerGgPdpGRS5IcGM4&usqp=CAE',
        type: 'cupboard',
        price: 70899,
        description: `Nordic wardrobe modern minimalist small apartment furniture storage bedroom economical door opener chef`,
        qty: 1
    },
    {
        id: 19,
        title: 'Modern minimalist living room bedroom floor lamp',
        image: 'https://encrypted-tbn0.gstatic.com/shopping?q=tbn:ANd9GcSR9j9qhMHKxjfOGGFRMsZVP-3AaamTUrV3BN_oC10SeFCFXIfZNQS1I1gYPdlVu7wDVw5kD7-BTlDHXLZiDX87nw1aiNEt0CtiMi3DUL8S&usqp=CAE',
        type: 'lighting',
        price: 16950,
        crossprice: '₹19,990',
        description: `Style: Modern Light source: Led 
        Body Material: Metal/Acrylic
        Body Color: As Shown Brand: N-Lighten Size: As Shown 
        Applicable place: Living room, Bedroom, Study room, Dining room, Hotel room/lobby, Exhibition hall`,
        qty: 1
    },
    {
        id: 20,
        title: 'Pendant Lamps Modern Ring Chandelier ',
        image: 'https://encrypted-tbn2.gstatic.com/shopping?q=tbn:ANd9GcRurTUxzZad_Smfbvf_-RYTx1pglf7CAEKWZwD88d5B74naXKDn7hc4QP891CzuXUMj_EmRqGZWyUDvqu8LnwJgNF4tU_aE99md90Nnzng&usqp=CAE',
        type: 'lighting',
        price: 14009,
        crossprice: '₹18,899',
        description: `Direct factory price, DHgate Professional Factory Shop! Lowest Price!Top Quality! Welcome to Retail and Wholesale! You can mix any items together from my store!!Notes: Due to the light and screen setting difference, the item's color may be slightly different from the pictures.`,
        qty: 1
    },
    {
        id: 21,
        title: 'Table Lamp',
        image: 'https://www.whiteteak.com/media/catalog/product/t/l/tl53-10001_10_.jpg?width=560&height=560&canvas=560,560&optimize=high&fit=bounds',
        type: 'lighting',
        price: 9940,
        crossprice: '₹10,499',
        description: `An enchantingly eccentric shape made of a metal frame in a matte brown finish with an acrylic diffuser, this dimmable LED table lamp is an absolute must-have for spaces with modern design sensibilities.`,
        qty: 1
    },
    {
        id: 22,
        title: 'Relax Natural Latex Mattresses',
        image: 'https://encrypted-tbn1.gstatic.com/shopping?q=tbn:ANd9GcTYKiyHtv_ytETUzUlhjX7GVQJwRXCts1573rFwJrMh2jB2ix0WTDpY_aO1yqAW81Y_xfrV1knISMa1NO5rG1MMRg58l19dvpZt1SiwSGs&usqp=CAE',
        type: 'mattresses',
        price: 25377,
        crossprice: '₹27,210',
        description: `Relax Natural Latex Mattresses are made from high-quality, eco-friendly materials to provide exceptional comfort and support while you sleep. The mattresses feature a natural latex foam core that conforms to your body shape, providing excellent pressure relief and spinal alignment.`,
        qty: 1
    },
    {
        id: 23,
        title: 'THE SERAPHIC CARAT A CHANDELIER',
        image: 'https://belacasalighting.com/cdn/shop/products/1_d27b05b2-a592-4c1b-8d0e-40f4584d0e70.jpg?v=1662626758',
        type: 'lighting',
        price: 105020,
        crossprice: '₹ 115,020',
        description: `NO. OF BULB: 26
        HOLDER: E14
        BULB PROVIDED: NO
        PRODUCT MATERIAL: METAL BRASS MATT FINISH WITH CRYSTALS `,
        qty: 1
    },
    {
        id: 24,
        title: 'Sheesham Wood Arm Chair In Passion Mahogany',
        image: 'https://homesofrajasthan.com/wp-content/uploads/2023/04/ch10-860x860.jpg',
        type: 'chair',
        price: 20.020,
        crossprice: '₹ 18,020',
        description: `NO. OF BULB: 26
        HOLDER: E14
        BULB PROVIDED: NO
        PRODUCT MATERIAL: METAL BRASS MATT FINISH WITH CRYSTALS `,
        qty: 1
    },
    {
        id: 25,
        title: 'Classic Sheesham Wood Arm Chair In Provincial Teak Finish',
        image: 'https://homesofrajasthan.com/wp-content/uploads/2023/04/ch19.jpg',
        type: 'chair',
        price: 105020,
        crossprice: '₹ 115,020',
        description: `NO. OF BULB: 26
        HOLDER: E14
        BULB PROVIDED: NO
        PRODUCT MATERIAL: METAL BRASS MATT FINISH WITH CRYSTALS `,
        qty: 1
    },
    {
        id: 26,
        title: 'Solid Wood 2- Door Wardrobe  Black Finish',
        image: 'https://homesofrajasthan.com/wp-content/uploads/2023/11/BSD-122-BLACK_WB1_20231122_153116406-860x860.jpg',
        type: 'lighting',
        price: 105020,
        crossprice: '₹ 115,020',
        description: `NO. OF BULB: 26
        HOLDER: E14
        BULB PROVIDED: NO
        PRODUCT MATERIAL: METAL BRASS MATT FINISH WITH CRYSTALS `,
        qty: 1
    },
    {
        id: 27,
        title: ' Wood Marble Top 8 Seater Dining Set With Chairs',
        image: 'https://homesofrajasthan.com/wp-content/uploads/2023/04/1-88-860x860.jpg',
        type: 'Dining table',
        price: 185020,
        crossprice: '₹ 115,020',
        description: `NO. OF BULB: 26
        HOLDER: E14
        BULB PROVIDED: NO
        PRODUCT MATERIAL: METAL BRASS MATT FINISH WITH CRYSTALS `,
        qty: 1
    },
    {
        id: 28,
        title: 'Solid Wood Rattan Black Finish Compact 2 Door Wardrobe',
        image: 'https://homesofrajasthan.com/wp-content/uploads/2023/11/FT-WARD-02-BLACK_WB1_600x_20231108_115344956-860x860.jpg',
        type: 'lighting',
        price: 155020,
        crossprice: '₹ 115,020',
        description: `NO. OF BULB: 26
        HOLDER: E14
        BULB PROVIDED: NO
        PRODUCT MATERIAL: METAL BRASS MATT FINISH WITH CRYSTALS `,
        qty: 1
    },
    {
        id: 29,
        title: 'THE SERAPHIC CARAT A CHANDELIER',
        image: 'https://belacasalighting.com/cdn/shop/products/1_d27b05b2-a592-4c1b-8d0e-40f4584d0e70.jpg?v=1662626758',
        type: 'lighting',
        price: 205020,
        crossprice: '₹ 115,020',
        description: `NO. OF BULB: 26
        HOLDER: E14
        BULB PROVIDED: NO
        PRODUCT MATERIAL: METAL BRASS MATT FINISH WITH CRYSTALS `,
        qty: 1
    },
    {
        id: 30,
        title: 'THE SERAPHIC Table',
        image: 'https://homesofrajasthan.com/wp-content/uploads/2023/04/1-17.jpg',
        type: 'table',
        price: 105020,
        crossprice: '₹ 115,020',
        description: `NO. OF BULB: 26
        HOLDER: E14
        BULB PROVIDED: NO
        PRODUCT MATERIAL: METAL BRASS MATT FINISH WITH CRYSTALS `,
        qty: 1
    },
    {
        id: 31,
        title: 'Swis kindom Sofa',
        image: 'https://homesofrajasthan.com/wp-content/uploads/2022/11/sauvage-chesterfield-leather-sofa-3-910x1155-1.jpg',
        type: 'Sofa',
        price: 105020,
        crossprice: '₹ 115,020',
        description: `NO. OF BULB: 26
        HOLDER: E14
        BULB PROVIDED: NO
        PRODUCT MATERIAL: METAL BRASS MATT FINISH WITH CRYSTALS `,
        qty: 1
    },
  
   
];
